...
export default function UserDetailScreen({ user, onBack, onSendHeartPing }: { user: RecommendedUser, onBack: () => void, onSendHeartPing: () => void }) {
  return (
    <div className="h-full w-full bg-[#0a0a0a] flex flex-col relative">
      ...
      <div className="h-2/5 bg-gradient-to-b from-[#1a1a2e] to-[#0a0a0a] flex items-center justify-center relative">
        <button onClick={onBack} className="absolute top-6 left-6 p-2 bg-black/20 backdrop-blur-md rounded-full text-white z-10">
          <ChevronLeft size={24} />
        </button>
        ...
        <MascotImage type={user.mascotType} className="w-56 h-56" />
      </div>
      ...
      <div className="flex-1 p-8 -mt-10 bg-[#0a0a0a] rounded-t-[40px] z-20 shadow-2xl shadow-black/80">
        <div className="flex justify-between items-start mb-6">
          <div>
            ...
            <p className="text-purple-400 font-medium">{user.department}</p>
          </div>
          <div className="text-right">
            <span className="block text-2xl font-bold text-pink-500">{user.matchScore}%</span>
        ...
        <div className="space-y-8">
          ...
          <section className="bg-[#1a1a1a] p-6 rounded-[32px] border border-gray-800">
            ...
            <p className="text-sm text-gray-400 leading-relaxed">
              안녕하세요! 저는 {user.nickname}입니다. 대화를 통해 천천히 알아가는 만남을 좋아해요. 축제 기간 동안 소소한 즐거움을 함께 나눌 분을 찾고 있습니다.
            </p>
          </section>
        </div>
      </div>
